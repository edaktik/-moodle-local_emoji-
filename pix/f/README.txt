...
1. Copy the plugin into /local/emoji/ folder of your Moodle 2.0 installation
...
Site administration > Appearance > HTML settings with the following values:
    ...
    +-------+---------------+-------------------+-----------+--------------+
    | :-(   | unhappy       | local_emoji      | sad       | core_pix     |
    +-------+---------------+-------------------+-----------+--------------+

The row says: the image for the emoticon :-( is called ''unhappy'' (with whatever
extension like .png, .gif or .jpg) and is located in /local/emoji/pix/
...
'local_emoji' respectively. Then the strings defined in
/local/emoji/lang/XX/local_emoji.php file are used.
...
How to localise alternative texts for emoticon images
-----------------------------------------------------

1. Copy the folder /local/emoji/lang/en/local_emoji.php into
   /local/emoji/lang/XX/local_emoji.php where XX is the language code
...
How to customise the alternative texts for emoticon images
----------------------------------------------------------

Go to Site administration > Language > Language customisation and modify the
...
Example: if you used ''unhappy'' and ''local_emoji'' as the alternative text
for the smiley, you have to modify the string ''unhappy'' in the local
component ''local_emoji.php''
